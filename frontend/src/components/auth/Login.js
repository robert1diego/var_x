...
export const EmailPassword = (
    ...
    password: {
        helperText: 'Su contrasena debe incluir minimo ocho caracteres,mayuscula,numero,y caracteres especiales',
...
export default function Login({
    ...
    const handleLogin = () => {
        ...
        axios
            ...
            .catch(error => {
                const { message } = error.response.data.message[0].message[0]
                setLoading(false)
                console.log(error)
                ...
                >
                    {loading ? <CircularProgress /> : (
                        <Typography variant="h5" classes={{ root: classes.buttonText }} >
                            {forgot ? "Reset contraseña" : "Entrar"}
                        </Typography>
                    )}
                </Button>
            </Grid>
